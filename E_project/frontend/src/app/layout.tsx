import type { Metadata } from "next";
import "./globals.css";
import Link from 'next/link';
import TopHeader from '@/components/TopHeader';

export const metadata: Metadata = {
  title: "ERP Project Manager",
  description: "Enterprise Resource & Project Management",
};

export default function RootLayout({
  children,
}: Readonly<{
  children: React.ReactNode;
}>) {
  return (
    <html lang="en">
      <body>
        <div className="app-wrapper">
          <aside className="sidebar">
            <div className="sidebar-brand">
              <svg width="36" height="36" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M12 2L20 6V18L12 22L4 18V6L12 2Z" stroke="white" strokeWidth="2" strokeLinejoin="round"/>
                <path d="M12 22V12M12 12L20 6M12 12L4 6" stroke="white" strokeWidth="2" strokeLinejoin="round"/>
                <path d="M9 14.5L15 10.5" stroke="white" strokeWidth="2" strokeLinecap="round"/>
              </svg>
              <span className="brand-name">Ondeck</span>
            </div>

            <hr className="sidebar-divider" />

            <div className="sidebar-header">
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M22 12h-4l-3 9L9 3l-3 9H2"></path></svg>
              MENUS
            </div>
            
            <nav className="sidebar-nav">
              <Link href="/" className="nav-item active">
                <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor" stroke="none"><path d="M10 3H3v7h7V3zm0 11H3v7h7v-7zm11-11h-7v7h7V3zm0 11h-7v7h7v-7z"/></svg>
                Dashboard
              </Link>
              <Link href="/tasks" className="nav-item">
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polygon points="12 2 2 7 12 12 22 7 12 2"></polygon><polyline points="2 17 12 22 22 17"></polyline><polyline points="2 12 12 17 22 12"></polyline></svg>
                My Tasks
              </Link>
              <Link href="/reports" className="nav-item">
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="12" y1="20" x2="12" y2="10"></line><line x1="18" y1="20" x2="18" y2="4"></line><line x1="6" y1="20" x2="6" y2="16"></line><line x1="2" y1="20" x2="22" y2="20"></line></svg>
                Reports
              </Link>
              <Link href="/calendar" className="nav-item">
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"></rect><line x1="16" y1="2" x2="16" y2="6"></line><line x1="8" y1="2" x2="8" y2="6"></line><line x1="3" y1="10" x2="21" y2="10"></line></svg>
                Calendar
              </Link>
              <Link href="/media" className="nav-item">
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z"></path></svg>
                Media Library
              </Link>
              <Link href="/messages" className="nav-item">
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"></path><polyline points="22,6 12,13 2,6"></polyline></svg>
                Messages
              </Link>
            </nav>

            <hr className="sidebar-divider mt-auto" />
          </aside>
          
          <main className="main-content">
            <TopHeader />
            {children}
          </main>
        </div>
      </body>
    </html>
  );
}
