...
export class BillingService {
        ...
        return this.prisma.institution.update({
            ...
            data: {
                ...
                ...(dto.subscriptionStartDate !== undefined && {
                    subscriptionStartDate: dto.subscriptionStartDate ? new Date(dto.subscriptionStartDate) : null,
                }),
                ...(dto.subscriptionEndDate !== undefined && {
                    subscriptionEndDate: dto.subscriptionEndDate ? new Date(dto.subscriptionEndDate) : null,
                }),
    ...
    async getBillingStats() {
        ...
        const statusCounts: Record<string, number> = { TRIAL: 0, ACTIVE: 0, SUSPENDED: 0, EXPIRED: 0 };

        institutions.forEach((inst) => {
            planCounts[inst.plan] = (planCounts[inst.plan] || 0) + 1;
            statusCounts[inst.subscriptionStatus] = (statusCounts[inst.subscriptionStatus] || 0) + 1;
        });

        const totalStudents = institutions.reduce((sum, inst) => sum + inst._count.users, 0);
        const avgStudents = institutions.length > 0 ? Math.round(totalStudents / institutions.length) : 0;
