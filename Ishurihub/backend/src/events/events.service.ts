...
import { SchoolEvent, EventType, TargetAudience } from './entities/school-event.entity';
import { EventOccurrence, OccurrenceStatus } from './entities/event-occurrence.entity';
...
export class EventsService {
    ...
    async create(createEventDto: CreateEventDto, schoolId: string): Promise<SchoolEvent> {
    ...
    private async generateOccurrences(event: SchoolEvent) {
        ...
        while (current <= end) {
            ...
            if (this.matchesPattern(current, pattern)) {
                await this.createSingleOccurrence(event, current.toISOString().split('T')[0]);
            }
    ...
    async recordAttendance(
        ...
        const occurrence = await this.occurrencesRepository.findOne({ where: { id: occurrenceId } });
